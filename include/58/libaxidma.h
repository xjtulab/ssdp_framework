 ...
 *
 * @param[in] dmadev_t    #The standard full name(with path) for the AXI DMA device 
 * @return A handle to the AXI DMA device on success, NULL on failure.
 **/
struct axidma_dev *axidma_init(char* dmadev_t);


/**
 * write axilite
 * @param[in] n_offset        #lite offset address 
 *                            #n_offset=0:set srio deviceid
 * @param[in] nValue          #writed  value  
 * @param[in] nBaseAddrNum    #0:DMA0   1:DMA1
 **/
void write_device_id( int nValue, unsigned char nBaseAddrNum); 
 ...
 * @
 **/
int read_device_id(unsigned char nBaseAddrNum);               
